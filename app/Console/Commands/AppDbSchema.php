...
class AppDbSchema extends AppBase
{
    ...
    public function handle() {
        $schema_php = ['<?php', ''];
        $schema_php[] = '/* In case of foreign key error, use commands above:';
        $schema_php[] = ' * SET FOREIGN_KEY_CHECKS=0;';
        $schema_php[] = ' * SET GLOBAL FOREIGN_KEY_CHECKS=0;';
        $schema_php[] = ' */';
        $schema_php[] = '';
        $schema_php[] = "\$database = env('DB_DATABASE');";

        $schema_sql = [];
        $schema_sql[] = '-- In case of foreign key error, use commands above:';
